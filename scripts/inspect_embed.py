...
def main(a):
    ...
    for idx in range(len(dset.dataset)):
        ...
        for idx in range(cos_sim_patch.shape[0]):
            ...
            col_start = max(0, col_idx - args.context_sz)
            col_end = min(n_patch_per_dim, col_idx + args.context_sz)
            
            row_idx = idx // n_patch_per_dim
            row_start = max(0, row_idx - args.context_sz)
            row_end = min(n_patch_per_dim, row_idx + args.context_sz)
