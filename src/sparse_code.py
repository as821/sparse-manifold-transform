...
class SparseCodeLayer:
    ...
    def __init__(self, dict_sz, gq_thresh, phi=None):
    ...
    def generate_dict(self, args, dset, dict_thresh):
        ...
        phi = torch.zeros(size=(patch_dim, self.dict_sz))
        if torch.cuda.is_available():
            phi = phi.to('cuda:0')
        shuf = [i for i in range(args.samples)]
        random.shuffle(shuf)
        pbar = tqdm(total=self.dict_sz)
        ...
        for start in range(0, len(shuf), chnk_sz):
            ...
            if ptr == self.dict_sz:
                print(f"Found {self.dict_sz} sufficiently (<={dict_thresh}) unique dictionary elements in {i} / {args.samples * dset.n_patch_per_img} ({i / (args.samples * dset.n_patch_per_img):0.4f}) tries.")
                break
        assert ptr == self.dict_sz, f"Unable to find {self.dict_sz} sufficiently (<={dict_thresh}) unique dictionary elements in the dataset (only found {ptr})."
        pbar.close()
        if torch.cuda.is_available():
            phi = phi.cpu()
